...
представлен не одним байтом, а четырмя.

Вы можете найти больше информации о символах (`char`) в [документации к
стандартной библиотеке][char].

[char]: ../std/primitive.char.html

# Числовые типы

Rust имеет целый ряд числовых типов, поделенных на несколько категорий: знаковые
...
представляет собой тип без знака с размером в шестнадцать бит. Большее
количетсво бит позволяет задавать большие числа.
...
давайте рассмотрим число с размером в четыре бита. Знаковые четырехбитные числа,
позволяют хранить значения от `-8` до `+7`. Знаковые цифры используют
представление ’дополнение до двух’. Беззнаковые четырехбитные числа, ввиду того
...
Типы с постоянным размером соответственно имеют фиксированное количество битов в
своем представлении. Допустимыми размерами являются `8`, `16`, `32`, `64`. Таким
образом, `u32` представляет собой целое число без знака с размером 32 бита, а
...
Как и в случае индексации массивов, индексы начинаются с нуля, но, в отличае от
