...
располагается ‘[шаблон][pattern]’, а не просто имя переменной. Это значит, что
...
очень мощны, и для них выделен отдельный [подраздел][pattern] в книге. Но в
настоящее время нам не нужны эти фичи, так что мы просто будем помнить об этом,
когда пойдем дальше.
...
есть такая вещь, как *вывод типов*. Если Rust самостоятельно может понять какой
тип у переменной, то он не требует указывать его.
...
Если бы я попросил вас прочитать это вслух, то вы должны были бы сказать "`x`
связан с типом `int` и имеет значение `пять`".

В этом случае мы указали что `x` у нас будет 32-битным целым числом со знаком. В
Rust есть и другие целочисленные типы. Они начинаются с `i` для целых чисел со
...
ознакомиться с [доступными опциями форматирования строк][format]. На данный
