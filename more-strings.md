...
доступ и выделение памяти для структур с динамическим размером включает в себя
...
Как и любая ссылка Rust, строковые срезы имеют связанный срок жизни. Для
строкового литерала он: `&'static str`. Строковый срез можно записать без явного
...
Как и векторные срезы, строковые срезы - это просто указатель плюс длина. Это
означает, что они являются 'видом' ('представлением') уже размещенной в памяти
строки, например, строкового литерала или `String`.

## `str`

Иногда вы можете встретить использование типа `str` без `&`. Хотя этот тип и
существует, но он не является тем типом, который вам следует использовать
самостоятельно. Иногда люди путают `str` и `String` и пишут так:
...
гарантированно является UTF-8 последовательностью. `String` обычно создаются
путем преобразования из строкового среза при помощи метода `to_string`.
...
Ссылка на `String` будет автоматически приводиться к строковому срезу:
...
использование `Vec<T>` vs `&[T]` или `T` vs `&T` в общем.
...
Чтобы записать функцию, обобщенную для обоих типов строк, используйте `&str`.
...
Как правило, метод `graphemes()` для `&str` - это именно то, что вам нужно:
