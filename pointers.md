...
1. Вы не можете выделить неправильное количество памяти для хранения объекта.
   Rust определяет размер по типу выделяемых данных.
...
# Счётчик ссылок и атомарный счётчик ссылок (Rc и Arc)
...
Заметьте, что вам придётся включить нестабильную возможность `box_syntax`. Данный
...
| `Rc<T>`      | Счётчик ссылок             | `T`, выделенный на куче, с множеством читателей                                      |
| `Arc<T>`     | Атомарный счётчик ссылок   | То же, что и выше, но безопасно для передачи в другие потоки                         |
