...
Мы используем `assert!` заявить, что что-то не верно. Если это не верно, что-то очень неправильно. Настолько неправильно, что мы не можем продолжать работу программы в текущем состоянии. Другой пример - использование макроса `unreachable!()`:
