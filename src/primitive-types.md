...
Для создания срезов  из различных вещей можно использовать комбинации `&` и
`[]`. `&` указывает на то, что срезы схожи с ссылками, а `[]`, с каким-либо
диапазоном, задаёт длину среза:
