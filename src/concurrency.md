...
связи с этим Rust дает нам два типажа, помогающих нам разбираться в коде,
...
реализует `Send`, это указывает компилятору, что право владения переменными
этого типа можно безопасно перемещать между потоками.
...
является потокобезопасным, то нам не следовало бы реализовывать типаж `Send`,
...
реализует `Sync`, это указывает компилятору, что переменные этого типа не имеют
возможности использовать небезопасную память, когда они используются из
...
Эти два типажа позволяют использовать систему типов, чтобы обеспечить надежные
...
предполагает, что его содержимое реализует типаж `Sync`. В нашем случае мы также
...
можем отправить по каналу любые данные, которые реализуют типаж `Send`!
