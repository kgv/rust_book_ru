...
Этот код создает новый экземпляр `Philosopher` и присваивает его полю `name`
значение переданного аргумента `name`. Но используется не сам аргумент, а
результат вызова его метода `.to_string()`. Этот вызов создает копию строки, на
которую указывает наш `&str` и возвращает новый экземпляр `String`, который и
будет присвоен полю `name` структуры `Philosopher`.

Почему бы сразу не передавать строку типа `String` напрямую? Так ведь легче ее
...
`Philosopher` и кажется, что ничего больше не делаем. Rust - это "выражение-
ориентированный" язык программирования, что означает, что каждое выражение
возвращает значение. Это верно и для функций, у которых автоматически
...
Имя функции `new()` не является спецификацией Rust, но негласным соглашением
принято так именовать функции, которые возвращают новые экземпляры структур.
Давайте снова посмотрим на функцию `main()`:
...
Здесь, мы связываем пять имен переменных с пятью новыми философами. Здесь
указаны имена некоторых известных философов, но вы можете указать любые другие.
Если бы мы _не объявили_ свою реализацию функции `new()`, то наш код выглядел бы
так:
...
имеет и другие преимущества, но даже в этом простом случае, его использование
было оправдано.
...
именуют как "вектор", он является расширяемой версией массива. Затем в цикле
[`for`][for] мы перебираем вектор, получая ссылку на очередного философа за
каждую итерацию.
...
```text
Джудит Батлер закончил(а) есть.
Жиль Делёз закончил(а) есть.
Карл Маркс закончил(а) есть.
Эмма Гольдман закончил(а) есть.
Мишель Фуко закончил(а) есть.
```
...
поэтому нам нужно объявить о его использование.
...
этими выводами. Эта функция останавливает рабочий поток на 1000 миллисекунд, что
...
```text
Джудит Батлер начал(а) есть.
Джудит Батлер закончил(а) есть.
Жиль Делёз начал(а) есть.
Жиль Делёз закончил(а) есть.
Карл Маркс начал(а) есть.
Карл Маркс закончил(а) есть.
Эмма Гольдман начал(а) есть.
Эмма Гольдман закончил(а) есть.
Мишель Фуко начал(а) есть.
Мишель Фуко закончил(а) есть.
```
...
собираемся создать несколько потоков, вследствие чего получим для них
дескрипторы, с помощью которых сможем контролировать их выполнение. Нам нужно
явно указать тип здесь, а для чего это необходимо, мы расскажем чуть позже. `_`
- это заполнитель типа. Мы говорим компилятору "`handles` - это вектор,
содержащий элементы, тип которых Rust должен вывести самостоятельно".
...
сообщает что это замыкание получает право владения на переменные, которые оно
...
какого-то типа, и для того чтобы Rust понял, какой тип коллекции мы хотим
...
Мьютекс - способ управления доступом к данным для параллельно выполняющихся
потоков: только один поток может получить доступ к данным в конкретный момент
времени. Это именно то, недвижимости нужно с нашими вилками. Мы используем
пустой кортеж, `()`, внутри мьютекса, так как мы на самом деле не собирается
использовать значение, просто удержать его.
...
программы. Приступим к рассмотрению:
...
блокированы, пока не станет вилке доступной.
...
произойти, потому как мы просто используем метод `unwarp()`.
...
соответственно. Зачем мы используем знаки подчеркивания в начале идентификаторов
имен? Это для того, чтобы сказать компилятору, что мы хотим получить значения,
которые далее _не планируем использовать_. Таким образом Rust не будет выводить
предупреждение о неиспользуемых идентификаторах имен.

Когда же мьютекс будет освобождён? Это произойдет автоматически, когда `_left` и
`_right` выйдут из области видимости, то есть по окончанию работы функции.
...
Посмотрите на последнюю строку создания `Philosopher`. Конструктор Мишель Фуко
должен бы иметь в качестве аргументов значения `4` и `0`, но вместо этого у нас
они `0` и `4`. Это помешает нашей программе попасть в безвыходное состояние,
если каждый возьмет по одной вилке одновременно. Так что давайте представим, что
один из философов у нас левша! Это один из способов решить данную проблему, и,
...
нас не было подсчета ссылок, то мы бы не знали, как и когда освободить его.
...
Теперь наша программа работает! Только два философа могут обедать одновременно и
