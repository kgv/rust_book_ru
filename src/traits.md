...
нашей обобщённой функции `inverse`?
...
метод `area`. Но мы можем добавить **ограничение по типажу** к нашему
обобщённому типу `T`, гарантируя, что он будет соответствовать требованиям:
...
Вернемся к разделу «[Обобщённое программирование](generics.html)». мы пытались
написать такой код:
...
При реализации типажа `Foo` необходимо реализовать метод `bar()`, но нет
