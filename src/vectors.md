...
[дженериком][generic]). Вектора всегда размещает данные в куче. Вы можете
...
Индексы отчситываются от `0`, так что третим элементом является `v[2]`.
...
У векторов есть много других полезных методов, о которых вы можете прочитать
[API документации][vec].
