...
на самом высоком месте в памяти. Поскольку структура `x` хранит [сырой указатель][rawpointer]
на адрес, который мы выделили в куче, то значение `x` равно (2<sup>30</sup>) - 1 —
это то самое местоположение в памяти.
