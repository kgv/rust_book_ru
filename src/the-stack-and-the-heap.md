...
Вот высокоуровневое сравнение: стек работает очень быстро; в Rust память
...
Вот, что мы имели ввиду раньше: наш стек растет вверх.
...
различными функциями или сохранить её правильность после окончания выполнения
...
потому что `T` как бы "упаковано" в толстый указатель `Box`: он знает размер
того, что лежит внутри. Эта информация закодирована в типе `T`, поэтому во время
исполнения, для размерных типов, это просто указатель.) Вот пример:
...
всё как обычно. Но что насчёт `x`? Наш `x` представляет собой `Box<i32>`, а
...
[drop]: drop.html
[moving]: Мы можем удлинить время жизни памяти путем передачи права
