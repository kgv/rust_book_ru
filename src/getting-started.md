...
Первым шагом к использованию Rust является его установка. В общем в этой главе
нам понадобится интернет соединение для выполнения команд, с помощью которых
мы загрузим Rust из интернета.

Мы воспользуемся несколькими командами в терминале, и они все будут начинаться
с `$`. Вам не нужно вводить `$`, они используются только для того что бы
обозначить начало каждой команды. Мы рассмотри множество руководств и примеров
по всему Интернету, которые следуют этой конвенции: `$` обозначает команды,
которые выполняются с правами обычного пользователя и `#` для команд, которые
...
Уровни поддержки Rust разбиты на три яруса, у каждого из которых свой набор
гарантий.

Платформы идентифицируются по их "целевой тройке", которая является строкой
сообщающая компилятору, какие выходные данные должны быть произведены. Столбцы
ниже указывают, работает ли соответствующий компонент на указанной платформе.

### Первый ярус

Первый ярус платформ может восприниматься как "гарантировано соберется и
заработает". В частности каждый из них удовлетворяет следующим требованиям:

 * Автоматическое тестирование настроено для тестирования этих платформ.
 * Изменения, принятые в master ветку `rust-lang/rust` репозитория, прошли
   тестирование.
 * Для этих платформ предоставляются пакеты официальных выпусков.
...
Второй ярус платформ может восприниматься как "гарантировано соберется".
Автоматические тесты не настроены и в связи с этим работоспособность сборки не
гарантируется. Но эти платформы обычно работаю довольно хорошо, и изменения
всегда приветствуются! В частности эти платформы удовлетворяют следующим
требованиям:

 * Настроена автоматическая сборка, но тестирования не происходит.
 * Изменения, принятые в master ветку `rust-lang/rust` репозитория, собираются
   для этих платформ. Имейте ввиду, что для некоторых платформ собирается только
   стандартная библиотека, но для остальных настроена полная сборка.
 * Для этих платформ предоставляются пакеты официальных выпусков.
...
Третий ярус платформ — это те, которые Rust поддерживает, но принятые изменения
автоматически не собираются и не тестируются. Рабочие сборки для этих платформ
могут быть изменены, для их надежности, на усмотрение сообщества. К тому же
официальные выпуски пакетов и установочников не предоставляются, но они могут
...
исчерпывающий набор третьего яруса!
...
Если вы используете Windows, то скачайте подходящий [установочник][install-page].

[install-page]: https://www.rust-lang.org/install.html

## Удаление

Удалить Rust также просто, как и установить его. На Linux или Mac нужно просто
...
Если вы использовали установочник для Windows, то вы можете повторно запустить
`.msi` файл и он предложит опцию для удаления.

## Решение проблем

Если у вас установлен Rust, то вы можете открыть терминал и ввести это:

```bash
$ rustc --version
```

Вы должны увидеть версию, хэш коммита и дату коммита.

Если это так, то теперь у вас есть установленный Rust! Поздравляем!

Если нет и вы используете Windows, то убедитесь в том, что Rust прописан в
...
терминал и введите следующие команды для создания директории для этого проекта:
...
Теперь давайте детально разберемся, что происходит в вашей "Hello, world!"
программе. Вот первый кусочек головоломки:
...
макросы Rust подробнее, на данный момент все, что вам нужно знать, — это то, что
...
В "Написание и запуск программы на Rust" мы рассмотрели как запустить только что
созданную программу. Сейчас мы рассмотрим каждый шаг по отдельности.
...
Cargo для управления их проектами на Rust. Cargo заботится о трех вещах: сборка
кода, загрузка библиотек, от которых зависит ваш код, и сборка этих библиотек.
Библиотеки, которые нужны вашему коду, мы называем "зависимостями"
...
Поскольку подавляющее количество проектов используют Cargo, будем считать, что
вы используете его для остальной части книги. Если вы использовали официальный
установщик, то Cargo установился вместе с Rust. Если же вы установили Rust
каким-либо другим образом, то вы можете проверить, есть ли у вас Cargo введя
...
на Cargo нужно сделать три вещи:
 ...
 1. Расположить файл с исходным кодом в нужной директории.
...
### Создание нового исполняемого файла и директории
...
Этот файл в формате *[TOML]* (Tom's Obvious, Minimal Language). TOML это аналог
INI, но с некоторыми дополнениями, и он используется в конфигурационном файле
для Cargo.
...
Другие три строчки устанавливают три значения конфигурации, которая необходима
Cargo для компиляции вашей программы: имя, версия и кто ее написал.
...
то можете использовать команду `cargo build --release` для компиляции вашего
проекта с оптимизацией. Эти оптимизации делают ваш Rust код быстрее, но требуют
больше времени на компиляцию. Именно из-за этого создано два разных профиля,
один для разработки, другой для сборки финальной версии, которую вы отдалите
пользователям.
...
## Простой способ сделать новый Cargo проект
...
не укажем этот аргумент, то Cargo создаст проект для библиотеки. Исполняемые файлы
частно называют *бинарниками* (как в `/usr/bin`, если вы используете Unix
систему).

Cargo сгенерировал два файла и одну директорию: `Cargo.toml` и директорию *src*
с файлом *main.rs*. Они должны выглядеть также, как те, что мы создали до этого.
