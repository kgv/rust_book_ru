...
> также [плагинов для разных редакторов][plugins], поддерживаемых комнадой Rust.
...
делается одной коммандой). В мире языков программирования много компромиссов, и
...
а также облегчит обмен кода с другими людьми и проектами. Далее мы познакомимся
...
релизу, то используйте команду `cargo build --release` для компиляции вашего
проекта с оптимизацией. Эти оптимизации делают ваш на Rust код быстрее, но
...
Rust-программиста. Более того, вы можете расчитывать, что практически проекты на
Rust можно будет собрать с помощью вариации этих комманд:
...
всего захотят пойти, пойдут по второму пути. Разные люди учатся по-разному!
