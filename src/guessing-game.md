...
Наша игра как раз и есть такой проект: нам надо быстро тестировать каждое
...
String`. В Rust есть такая особенность, называемая ["ссылки"][references]. Она
позволяет нам иметь несколько ссылок на одни и те же данные, благодаря чему
можно избежать излишнего их копирования. Ссылки - достаточно сложная
особенность, и одним из основных подкупающих достоинств Rust является то, как он
решает вопрос безопасности и простоты их использования. Пока что у нас нет
необходимости знать об этих деталях, чтобы завершить нашу программу. Сейчас,
всё, что нам нужно - это знать что ссылки, как и связывание при помощи `let`, не
изменяемо по умолчанию. Следовательно, мы должны написать `&mut guess`, а не
`&guess`.

Почему `read_line()` получает изменяемую ссылку на строку? Его работа - это
взять то, что пользователь написал в стандартный ввод, и положить это в строку.
Итак, функция получает строку в качестве аргумента, и для того, чтобы добавить в
эту строку что-то, она должна быть изменяемой.
...
значение, для которого он вызван, и если оно не удачное, выполняет [`panic!`]
[panic] с сообщением, указанным методу в качестве аргумента. `panic!` остановит
нашу программу и выведет сообщение об ошибке.
...
правильным решением для предотвращения ошибки будет её обработка. К счастью,
если мы только хотим обрушить приложение если есть проблема, мы можем
использовать эти два небольших метода. Если мы можем восстановить что-либо из
ошибки, мы должны сделать что-либо другое, но мы сохраним это для будущего
проекта.
...
[контейнер `rand`][randcrate]. "Контейнер" - это пакет с кодом Rust. Наш
проект - "бинарный контейнер", из которого в итоге получится исполняемый файл.
`rand` - "библиотечный контейнер", который содержит код, предназначенный для
...
публиковать свои проекты с открытым исходным кодом для возможности их
использования в других проектах.

[cratesio]: https://crates.io

После обновления реестра, Cargo проверяет секцию `[dependencies]` и скачивает
...
содержащая важные изменения? Что если исправления на столько масштабны, что
версия `0.3.9` становится не совместимой с нашим кодом?
...
использовать контейнер версии `0.3.8` до тех пор, пока явно не обновим
...
`0.4.x`, мы должны указать это в `Cargo.toml`. Потом, когда мы запустим
`cargo build`, Cargo обновит индекс и пересмотрит наши требования к `rand`.

В [документации][doccargo] по Cargo можно узнать о нём, а так же о
[его экосистеме][doccratesio] на много больше, но пока что это всё, что нам
нужно знать. Cargo делает повторное использование библиотек на много проще, и
растацеане, как правило, пишут небольшие проекты, которые входят в состав других
...
`extern crate rand`. Так как мы указали `rand` в секции `[dependencies]`, мы
можем использовать `extern crate` для того, чтобы Rust знал, что мы собираемся
использовать эту зависимость. Это так же эквивалентно `use rand;`, теперь мы
можем использовать всё, что есть в контейнере `rand`, используя префикс
`rand::`.
...
Вторая строка печатает наш секретный номер. Это поможет нам во время
...
с предполагаемым ответом. Здесь приведён наш следующий шаг, который, к
...
Метод `cmp()` может быть вызван у чего-либо, что может быть сравнимым, и
получает ссылку на что-либо с чем мы хотим сравнивать. Результатом сравнения
будет тип `Ordering`, который мы добавили выше. Мы используем оператор
[`match`][match] для определения `Ordering ` - результата сравнения.
`Ordering` - [`перечисление`][enum]. Они обозначаются как `enum`, сокращённо от
...
Оно может иметь тип `i32` - тридцати двух битное целое, или `u32` - тридцати
двух битное целое без знака, или `i64` - шестидесяти четырёх битное целое, или
какой-нибудь другой. По умолчанию, Rust сделает его тридцати двух битным целым,
`i32`. Однако, здесь Rust не знает как сравнить `guess` и `secret_number`. Одни
должны быть одного типа. В итоге, чтобы можно было сравнить `guess` и
`secret_number`, мы должны преобразовать переменную `guess`, которую мы
прочитали с ввода, из типа `String` в число вещественного типа. Мы можем сделать
это добавив несколько строчек. Вот как будет выглядеть наша программа:
...
Последуем за вызовами `ok().expect()`. Здесь `guess` ссылается на старый
...
идущее после `guess`, говорит Rust, что мы указали тип привязки. `u32` -
тридцати двух битное беззнаковое целое число. У Rust есть [несколько встроенных
числовых типов][number], но мы выбрали именно `u32`. Это достаточно хороший тип,
что бы хранить небольшие положительные числа.
...
так же означает и завершение нашей программы, так как это последнее, что есть в
...
вернёт в привязку `guess`. Если произойдёт какая-то ошибка (`Err`), то мы не
сможем определить её вид, поэтому мы используем вместо имени `_`. Так мы
проигнорируем ошибку и вызовем `continue`, который отправит нас на следующую
итерацию цикла.
