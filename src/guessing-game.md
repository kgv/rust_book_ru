...
программистов задачу - создадим игру-угадайку. Немного о том, как игра должна 
работать: наша программа генерирует случайное целое число из промежутка от 1 до 
100. Затем она просит ввести число, которое она "загадала". Для каждого 
введённого нами числа, она говорит, больше ли оно, чем "загаданное", или меньше. 
Игра заканчивается когда мы отгадываем число. Звучит не плохо, не так ли?
...
Великолепно! Команда `run` помогает, когда надо быстро проверить работу проекта. 
Наша игра как раз и есть такой проект: нам надо быстро тестировать каждое изменение,
...
может быть не просто имя связи, а [образец][patterns]. Мы будем использовать образцы
дальше. Связи достаточно просто использовать:
...
В Rust есть такая особенность, называемая ["ссылки"][references]. Она позволяет 
нам иметь несколько ссылок на одни и те же данные, благодаря чему можно
...
безопасности и простоты их использования. Пока что у нас нет необходимости знать 
об этих деталях, чтобы завершить нашу программу. Сейчас, всё, что нам нужно - 
это знать что ссылки, как и связывание при помощи `let`, не изменяемо по 
...
он вызван, и если оно не удачное, выполняет [`panic!`][panic] с сообщением, 
указанным методу в качестве аргумента. `panic!` остановит нашу программу и 
...
правильным решением для предотвращения ошибки будет её обработка. К счастью, если
...
мы указали `guess`. Если у нас есть несколько указателей места заполнения, мы 
должны указать несколько аргументов:
...
Однако, для получения этого функционала разработчики Rust предоставили 
[контейнер `rand`][randcrate]. "Контейнер" - это пакет с кодом Rust. Наш проект - 
"бинарный контейнер", из которого в итоге получится исполняемый файл. `rand` - 
"библиотечный контейнер", который содержит код, предназначенный для использования 
...
Что-то новенькое! Теперь, когда у нас есть внешние зависимости, Cargo скачал
...
публиковать свои проекты с открытым исходным кодом для возможности их использования
в других проектах.

[cratesio]: https://crates.io

После обновления реестра, Cargo проверяет секцию `[dependencies]` и скачивает всё,
что нам необходимо. В нашем случае, мы сказали, что наш проект зависит от `rand`.
Самому контейнеру `rand` для работы нужен контейнер `libc`. По этой причине Cargo
скачал и `libc`. После загрузки всего необходимого, оно компилируется, а затем
...
проект, и все его зависимости, а значит нет никаких причин делать это снова. Раз
делать ничего не надо, Cargo просто завершил работу. Если мы снова откроем файл
...
`0.3.x`, и он взял последнюю версию, на тот момент, когда его запустили - `v0.3.8`.
Но что делать, когда на следующей неделе выйдет версия `v0.3.9`, содержащая
важные изменения? Что если исправления на столько масштабны, что версия `0.3.9`
становится не совместимой с нашим кодом?

Решением этой проблемы является файл `Cargo.lock`, который мы можем найти в
...
ли `Cargo.lock`, и затем использовать указанные в нём версии контейнеров. Это
помогает нам автоматизировать повторяющиеся сборки. Другими словами, мы будем
использовать контейнер версии `0.3.8` до тех пор, пока явно не обновим информацию
о его версии в `Cargo.lock`.

А что, если мы захотим использовать версию `v0.3.9`? У Cargo есть другая команда,
`update`, которая скажет "игнорируй Cargo.lock, найди последние версии библиотек
из той ветки, которую мы указали в Cargo.toml. Когда всё сделаешь, запиши
информацию о версиях в Cargo.lock". Но по умолчанию, Cargo смотрит только версию
больше, чем `0.3.0`, и меньше `0.4.0`. Если мы хотим перейти на версии `0.4.x`,
мы должны указать это в `Cargo.toml`. Потом, когда мы запустим `cargo build`,
Cargo обновит индекс и пересмотрит наши требования к `rand`.

В [документации][doccargo] по Cargo можно узнать о нём, а так же о [его экосистеме][doccratesio]
на много больше, но пока что это всё, что нам нужно знать. Cargo делает повторное
использование библиотек на много проще, и растацеане, как правило, пишут небольшие
проекты, которые входят в состав других более крупных проектов.
...
crate rand`. Так как мы указали `rand` в секции `[dependencies]`, мы можем
использовать `extern crate` для того, чтобы Rust знал, что мы собираемся
использовать эту зависимость. Это так же эквивалентно `use rand;`, теперь мы можем
использовать всё, что есть в контейнере `rand`, используя префикс `rand::`.

Далее, мы добавили новую строку `use`: `use rand::Rng`. Это сделано для того,
чтобы метод `Rng` был в области видимости. Основная идея такова: методы,
объявленные где-то в другом месте, называются "типажами", и для того, чтобы этот
метод можно было использовать, необходимо чтобы типаж был в области видимости.
Чтобы узнать об этом более подробно, можно прочитать секцию о [типажах][traits].
...
Мы используем функцию `rand::thread_rng()` для получения копии генератора случайных
чисел, который будет локальным для текущего [потока][concurrency] выполнения.
Выше мы добавили `use rand::Rng` и теперь можем использовать метод `gen_range()`.
Этот метод получает два аргумента и генерирует число, которое может быть больше
либо равно первому аргументу и меньше, чем второй аргумент. Таким образом, если
мы укажем числа 1 и 101, то от генератора можно получить числа от 1 до 100
включительно.

[concurrency]: concurrency.html

Вторая строка печатает наш секретный номер. Это поможет нам во время тестирования,
пока мы разрабатываем нашу программу. Но мы обязательно удалим эту строчку в
финальной версии. Будет не интересно играть в игру, если она сразу печатает ответ!
...
Теперь, когда мы знаем что ввёл пользователь, давайте сравним "загаданное" число
с предполагаемым ответом. Здесь приведён наш следующий шаг, который, к сожалению,
не будет работать:
...
Метод `cmp()` может быть вызван у чего-либо, что может быть сравнимым, и получает
ссылку на что-либо с чем мы хотим сравнивать. Результатом сравнения будет тип
`Ordering`, который мы добавили выше. Мы используем оператор [`match`][match] для
определения `Ordering ` - результата сравнения. `Ordering` - [`перечисление`][enum].
Они обозначаются как `enum`, сокращённо от `enumeration` (перечисление).
...
предлагает вам создать "рукава" для каждого возможного значения. Так как у нас
есть три возможных значения `Ordering`, у нас будет три рукава:
...
Фью! Это большая ошибка. Ключом этой ошибки является "несоответствие типов".
В Rust строгая статическая система типов. Однако, он так же содержит систему
выведения типов. Когда мы пишем `let guess = String::new()`, Rust понимает, что
`guess` должна быть типа `String`, благодаря чему мы можем не указывать тип явно.
`secret_number` - число, которое может иметь значение от одного до ста. Оно может
иметь тип `i32` - тридцати двух битное целое, или `u32` - тридцати двух битное
целое без знака, или `i64` - шестидесяти четырёх битное целое, или какой-нибудь
другой. По умолчанию, Rust сделает его тридцати двух битным целым, `i32`. Однако,
здесь Rust не знает как сравнить `guess` и `secret_number`. Одни должны быть одного
типа. В итоге, чтобы можно было сравнить `guess` и `secret_number`, мы должны
преобразовать переменную `guess`, которую мы прочитали с ввода, из типа `String`
в число вещественного типа. Мы можем сделать это добавив несколько строчек.
...
Подождите минутку, у нас ведь уже есть `guess`? Rust позволил нам "перекрыть" 
(скрыть) предыдущую `guess` новой. Это часто используется в подобных случаях,
когда `guess` изначально бывает типа `String`, но нам требуется преобразовать её
в `u32`. Скрытие позволяет нам переиспользовать имя `guess`, а не создавать для 
каждого типа привязку с уникальным именем, таким как `guess_str` и `guess` или
каким-нибудь другим.
...
Последуем за вызовами `ok().expect()`. Здесь `guess` ссылается на старый `guess`,
который ещё является строкой, которую мы получили с ввода. Метод `trim()` у типа
`String` удаляет всё пустое пространство с начала и конца нашей строки. Это важно, 
ведь для нормальной работы `read_line()` нам необходимо нажать `Enter` после 
окончания ввода. Это значит, что если мы набрали `5` и нажали `Enter`, `guess` 
выглядит следующим образом: `5\n`. `\n` обозначает "новую строку" (newline) - 
значение клавиши `Enter`. `trim()` удалит его и оставит только `5`. [Метод 
`parse()`][parse], применяемый к строке, преобразует её в число. Он может 
анализировать различные числа, но мы можем указать Rust какой именно тип нам 
нужен. Поэтому мы указали `let guess: u32`. Двоеточие `:`, идущее после `guess`,
говорит Rust, что мы указали тип привязки. `u32` - тридцати двух битное 
беззнаковое целое число. У Rust есть [несколько встроенных числовых типов][number],
но мы выбрали именно `u32`. Это достаточно хороший тип, что бы хранить небольшие
положительные числа.
...
С добавлением строки `break` после вывода `Вы выиграли!`, мы получили возможность
выхода из цикла когда мы угадали загаданное число. Выход из цикла так же означает
и завершение нашей программы, так как это последнее, что есть в `main()`. Нам 
надо сделать ещё одно улучшение - при любом не числовом вводе, мы не должны 
выходить из программы, мы просто должны проигнорировать ввод. Мы можем сделать 
...
`Err` - ошибку. У каждого есть некоторая дополнительная информация: преобразованное
число, либо тип ошибки. Здесь мы проверили значение результата работы `parse()`
при помощи `match`. В случае, если результат равен `Ok`, то `match` привяжет 
внутреннее значение результата (`Ok(num)`) к имени `num` и вернёт в привязку 
`guess`. Если произойдёт какая-то ошибка (`Err`), то мы не сможем определить её вид,
поэтому мы используем вместо имени `_`. Так мы проигнорируем ошибку и вызовем 
`continue`, который отправит нас на следующую итерацию цикла.

Сейчас всё должно быть нормально! Давайте посмотрим:
...
Замечательно! При помощи этих небольших улучшений, мы закончили нашу игру. Что 
вы думаете об этом? Всё правильно, мы не должны видеть наше секретное число.
Знание этого числа хорошо для тестирования, но его знание во время игры не 
интересно. Так выглядит окончательный вариант нашего кода:
...
Теперь у вас есть успешно созданная игра "Угадайка"! Поздравляем!
