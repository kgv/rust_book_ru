...
Rust импортирует в нашу программу лишь самые необходимые вещи — "[вступление]"
[prelude] (prelude). Если чего-то нет во "вступлении", мы должны при помощи
`use` явно указать, что хотим это использовать. Здесь также присутствует второе
вступление — [`io` вступление][ioprelude], оно выполняет следующую функцию:
когда вы его импортируете, оно импортирует ряд полезных вещей, связанных с `io`.
...
`rand="=0.3.0"` (два знака равно). И если мы хотим использовать последнюю версию
