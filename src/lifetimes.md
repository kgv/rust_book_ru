...
Как вы можете видеть, мы объявили то же время жизни, что и для `Foo`, в строке
с `impl`. Мы повторяем `'a` дважды, как в функциях: `impl<'a>` устанавливает
время жизни `'a`, и `Foo<'a>` использует его.
...
другом, и что он возвращает значение в ту же область видимости. Если вы хотите,
чтобы `x` и `y` имели разные области видимости, вы должны использовать
параметры нескольких времён жизни (multiple lifetimes):
...
В этом примере `x` и `y` имеют разные действующие области видимости, но
возвращаемое значение имеет то же время жизни, что и `x`.
