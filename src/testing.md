...
`should_panic` тесты могут быть хрупкими, поскольку трудно гарантировать, что
...
функцию с известными аргументами и сравнить результат её вызова с ожидаемыми
результатом.
...
обычно используют возможность `glob`. Давайте, в соответствии с этим, изменим
`src/lib.rs`:
...
тестирование небольшего кусочка функциональности, имеет смысл перенести в этот
...
документация была написана. Для того, чтобы такой ситуации не возникало, Rust
поддерживает автоматический запуск примеров в документации. Вот дополненный
`src/lib.rs` с примерами:
