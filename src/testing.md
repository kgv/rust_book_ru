...
Мы также получаем ненулевой код состояния. Мы можем использовать `$?` на OS X и Linux:
...
Иногда несколько специфичных тестов могу занимать много времени на выполнение.
Такие тесты могут быть отключены по умолчанию с помощью атрибута `ignore`:
...
Теперь мы запускаем наши тесты и видим, что `it_works` работает, а
`expensive_test` нет:
...
смотрите раздел о [документации](documentation.html).

Последнее замечание: тесты в документации *не работают* для исполняемых файлов.
Подробнее об организации файлов смотрите в разделе [контейнеры и модули](crates-and-modules.html).
