...
самых сильных сторон в Rust: фактическое отсутствие рантайма.

По мере роста организации, они все больше полагаются на множество языков
программирования. У каждого языка программирования есть свои сильные и слабые
стороны, и полиглотное знание стека языков позволяет использовать конкретный
язык там, где проявляется его сильные стороны, и использовать другой язык там, где
он слабые.

Существует много областей, где многие языки программирования слабы в плане
производительности выполнения программ. Часто стоит компромисс использования
языка, который медленнее, но способствует повышению производительности
программиста. Чтобы смягчить эту проблему, часть кода системы можно написать на
C, а затем вызвать этот код, написанный на C, как если бы он был написан на
языке высокого уровня. Это называется "интерфейс внешних функций" (foreign
function interface), часто сокращается до "FFI".
...
отсутствие сборщика мусора и низкие требования к рантайму (времени выполнения),
что делает Rust отличным кандидатом на роль встраиваемого в другие языки
(вызываемого из других языков), когда нужно некоторые дополнительные
возможности.
...
многими другими языками: числовые вычисления и многопоточность.

Во многих языках, в целях согласованности (связности), числа размещаются в куче,
а не в стеке. Особенно в языках, которые сосредотачиваются на объектно-
ориентированном программировании и использовании сборщика мусора, по умолчанию
память выделяется из кучи. Иногда, при оптимизации, для конкретных чисел память
может выделяться в стеке, но вместо того, чтобы полагаться на работу
оптимизации, мы можем захотеть убедиться в том, что мы используем примитивные
типы чисел, а не какой-либо тип объекта.

Во-вторых, многие языки имеют "глобальную блокировку интерпретатора" ("global
interpreter lock"), который ограничивает параллелизм во многих ситуациях. Это
...
> миллионов. После того как все десять потоков завершаться, напечатать
> "сделано!".
...
Хотя это и синтетическая (надуманная, игрушечная) программа, на ее примере можно
продемонстрировать много проблем, аналогичных этой, характерных для реального
мира. Для наших целей, раскручивающиеся занятые потоки представляют собой
параллельные, требующие больших затрат, вычисления.
...
Первое предупреждение происходит, потому что мы строим библиотеку. Если бы у нас
был тест для этой функции, то предупреждение бы пропало. Но сейчас, она никогда
не вызывается.

Второе предупреждение связано с использованием `x` против `_x`. Так как мы не
...
В конце мы присоединяемся к каждому потоку.
...
должны сделать, изменить начало нашего кода:
...
библиотеку совместно используемых объектов (разделяемую библиотеку), написанную
на C! Попутно следует отметить, это может быть `embed.dll` или `libembed.dylib`,
...
Прежде чем мы сможем запустить этот код, нам нужно установить `ffi` gem:
...
Первый делом, нам надо объявить `ffi` джем. Он предоставляет нам интерфейс для
...
Автор `ffi` джема рекомендует использовать модуль, чтобы ограничить область
...
Метод `attach_function` предоставляется джемом `FFI`. Здесь соединяются наша
...
к `attach_function` устанавливает все это до. Это выглядит как функция Ruby, но
на самом деле это Rust!
