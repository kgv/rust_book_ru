...
Чтобы делать это, вам для начала необходимо узнать имя файла, которое как правило
...
отладку проще, но это также вынуждает нас требовать от типа-параметра `E`
...
библиотеке, вы могли бы быть небезосновательно разгневаны. Так что нам стоит
...
Мы хотим сказать, *почему* парсинг не удался. Вам стоит рассуждать похожим
...
является обобщенным (generic) над всеми различными типами чисел из
...
[концепцию с псевдонимом типа `Result`](#Создание-псевдонима-типа-result),
...
[предыдущем разделе](#Объявление-собственного-типа-ошибки):
...
конструктор значения. *Как правило*, это все что нужно.
...
[совмещении ошибок](#Настоящий-макрос-try). Реализация типажа `Error` также
...
[совмещать более детальные ошибки](#Совмещение-собственных-типов-ошибок). Например,
...
[псевдоним типа `Result`](#Создание-псевдонима-типа-result), особенно, если в вашей
