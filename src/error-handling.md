...
когда неизбежное произойдет. Rust имеет богатую поддержку обработки ошибок,
которые могут (давайте будем честны: будут) происходить в ваших программах.

Есть два основных типа ошибок, которые могут возникнуть в программе: неудачи
(failures), и паники (panics). Давайте поговорим о разнице между ними, а затем
обсудим, как справиться с каждым из типов ошибок. Затем мы обсудим
преобразование неудачи в панику.

# Неудача и паника

Rust различает два типа ошибок: неудачи и паники. *Неудача* - это ошибка, после
которой работоспособность программы может быть каким-либо способом
восстановлена. А *паника* - это ошибка, после которой работоспособность
программы восстановлена быть не может.

Что мы подразумеваем под понятием "восстановлена"? В большинстве случаев,
...
Это не сработает. Итак, мы знаем, что эта функция будет работать только при
некоторых входных данных. Это ожидаемое поведение. Мы называем такой тип ошибок
*неудача*.

С другой стороны, есть некоторые ошибки, которые являются неожиданными, или
после которых мы не можем восстановить работоспособность программы. Классическим
...
Простейший способ указать, что функция может потерпеть неудачу - это
использовать тип `Option<T>`. Например, метод `find` для строк пытается найти
шаблон в строке и возвращает `Option`:
...
определять его самостоятельно, чтобы использовать в своем коде. Вариант `Ok(T)`
представляет собой успех, а вариант `Err(E)` представляет собой неудачу.
...
восстановлена работоспособность программы, используется макрос `panic!`, который
вызывает панику. Это приведет к аварийному завершению текущего потока, и выдаст
сообщение об ошибке:
...
```text
thread '<main>' panicked at 'бабах', hello.rs:2
```
...
последующей попыткой восстановления работоспособности программы, но он
значительно короче. Хотя в некоторых ситуациях аварийное завершение программы
более целесообразно.
...
Выражение, обёрнутое в `try!`, вернёт распакованное успешное значение (т.е. `T`
из `Ok(T)`), если результат не `Err`. В противном же случае нераспакованное
`Err(E)` будет возвращено вызывающему.

Стоит отметить, что вы можете использовать `try!` только в функции, которая
возвращает `Result`. Это значит, что вы не можете использовать `try!` внутри
функции `main()`, потому что `main()` ничего не возвращают.

`try!` использует
[`From<Error>`](http://doc.rust-lang.org/std/convert/trait.From.html), чтобы
