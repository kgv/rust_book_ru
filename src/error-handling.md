...
способом обрабатывать ошибки. Вообще говоря, существует два общих подхода
...
Эта глава очень длинная, главным образом потому, что мы начнем с самого начала
— рассмотрения тип-сумм (sum type) и комбинаторов, и далее попытаемся
...
* [Основы](#the-basics)
    ...
    * [Тип `Result`](#the-result-type)
        * [Парсинг чисел](#parsing-integers)
        * [Создание псевдонима типа `Result`](#the-result-type-alias-idiom)
    * [Короткая интерлюдия: `unwrap` — не обязательно зло](#a-brief-interlude-unwrapping-isnt-evil)
...
* [В конечном итоге (TL;DR)](#the-short-story)
...
код пригодным для компонования (composability).

Сохранять код пригодным к компонированию важно потому, что без этого требования
...
текущего потока и, в большинстве случаев, к завершению всей программы.)
Вот пример:
...
нестись в направлении, которое ему вздумается, и растаптывать все на своем пути.
...
`Some` — это вариант или *конструктор по значению* для типа `Option`.
Его можно интерпретировать как функцию типа `fn<T>(value: T) -> Option<T>`.
Соответственно, `None` — это также конструктор по-значению, только у него нет
...
Но подождите, как на счет `unwrap`, который мы [`до этого`](#code-unwrap-double)
...
не пригоден для compasibility кода: это слон в посудной лавке.
...
случае, когда в имени файла отсутствует расширение. С другой стороны, каждый
...
В действительности, `map` [определен как метод][2] `Option<T>` в стандартной
библиотеке.
...
К примеру, ваша программа может считать, что расширение файла `rs` в случае,
если на самом деле оно отсутствует.
...
стандартной библиотеке, так что мы воспользовались ним вместо функции, которую
...
этом разделе связанная с определением расширения заданного имени файла. Чтобы
делать это, вам для начала необходимо узнать имя файла, которое как правило
извлекается из *файлового пути*. Хотя большинство файловых путей содержат имя
файла, подобное нельзя сказать обо *всех* файловых путях. Прмером могут
послужить пути `.`, `..` или `/`.
...
Можно подумать, мы могли бы просто использовать `map` комбинатор, чтобы
...
compasibility, поскольку они позволяют вызывающему обрабатывать возможность
...
возможность *отсутствия* подобно `Option`, `Result` выражает возможность
...
Тип `Result` — это способ выразить один из двух возможных исходов в вычисления.
...
отладку проще, но это также вынуждает нас требовать от параметрического типа `E`
(который представляет наш тип ошибки) реализации [`Debug`][8]. Поскольку
подавляющее большинство типов должны реализовывать `Debug`, такое ограничение
имеет тенденцию работать на практике. (Реализация `Debug` для некоторого типа
просто означает, что существует разумный способ печати удобночитаемого описания
значения этого типа.)
...
в целые числа. По факту это настолько просто, что возникает соблазн написать
что-то вроде:
...
В этом месте вы должны быть скептически настроены по-поводу вызова `unwrap`.
...
библиотеке, вы могли бы быть не безосновательно разгневаны. Так что нам стоит
...
Мы хотим сказать, *почему* парсинг не удался. Вам стоит попытаться подражать
этим рассуждениям, когда вы сталкиваетесь с выбором между `Option` и `Result`.
Если вы можете предоставить подробную информацию об ошибке, то вам, вероятно,
следует это сделать. (Позже мы поговорим об этом детальнее.)

Хорошо, но как мы запишем наш тип возвращаемого значения? Метод `parse`,
является общенным (generic) над всеми различными типами чисел из
...
список общих комбинаторов между `Result` и `Option`. И `map` входит в этот
список:
...
поскольку `Result` имеет второй параметрический тип, существуют комбинаторы,
...
предыдущий пример с парсингом целых чисел можно переписать так:
...
Наиболее заметное место использование такого подхода в стандартной библиотеке —
...
## Короткая интерлюдия: `unwrap` — не обязательно зло

Если вы внимательно следили, то возможно заметили, что я занял довольно жесткую
позицию по отношению методам вроде `unwrap`, которые могут вызвать `panic` и
прервать исполнение вашей программы. *Вообще говоря*, это хороший совет.

Тем не менее, `unwrap` все-таки можно использовать разумно. Факторы, которые
оправдывают использование `unwrap` являются несколько туманным и разумные люди
...
ровно то же самое, что и `unwrap`, за исключением того, что в случае паники
...
Пока что я говорил о комбинаторах определенных для `Option` и комбинаторах
определенных для `Result`. Эти комбинаторы можно использовать для того, чтобы
сочитать результаты различных вычислений, не делая подробного вариативного
анализа.
...
то время как `arg.parse()` возвращает `Result`. Они не могут быть скомпонированы
...
требует явного определения, какую именно ошибку использовать в случае, когда
...
[`Result::map_err`](http://doc.rust-lang.org/std/result/enum.Result.html#method.map_err). Это тоже
самое, что и `Result::map`, за исключением того, функцию применяется к
...
процедуры парсинга как примеры обработки ошибок.
...
(N.B. Мы используем `AsRef` по [тем же причинам, почему он используется в
...
3. Проблема при парсинге данных в число.
...
[увидете псевдоним типа](http://doc.rust-lang.org/std/io/type.Result.html), и следовательно, лежащий
в основе тип `io::Error`.) Третяя проблема определяется типом
...
Давайте начнем рефакторинг функции `file_double`. Для того, чтобы эта функция
была компонируема с остальным кодом, она *не должна* паниковать, если какие-либо
из перечисленных выше условий об ошибке будут выполнены. Фактически, это
...
прежде вы сможете писать такое. Написание кода в подобном стиле называется
...
Комбинатор `and_then` используется для объединения нескольких вычислений по
цепочке, где каждое вычисление может вернуть ошибку. После открытия файла,
есть еще два вычисления, которые могут завершится неудачей: чтение из файла и
парсинг содержимого в виде числа. Соответственно, имеем два вызова `and_then`.
...
Давайте попробуем другой подход: в преждевременный возврат.

## Преждевременный `return`

Я хотел бы взять код из предыдущего раздела и переписать его с применением
...
Некоторые могут обосновано не согласиться по поводу того, что этот код лучше,
...
мы получаем уплотненный код, который легче читать:
...
[наше определение `try!`](#code-try-def-simple), поскольку типы ошибок все еще
...
примерах удобен потому, что достаточно легко конвертировать любые ошибки в
...
вставлять сообщения об ошибке прямо код. На самом деле, мы именно этим и
...
чреват *потерей информации*. Другими словами, если все ошибки будут
...
исследовать тип ошибки с помощью вариативного анализа, и это значительно
превосходит попытку вычленить детали об ошибке из `String`.
...
Идеальным способом представления *один из многих вариантов* является
...
используя соответствующий конструктор по значению:
...
реализованы для *всех* типов, которые представляют собой ошибки. Как мы увидим
далее, он нам очень пригодится для написания компонуемого кода. Этот типаж
как минимум позволяет выполнять следующие вещи:

* Получать строковое представление об ошибке для разработчика (`Debug`).
* Получать понятное для пользователя представление об ошибке (`Display`).
...
[типаж-объекты](trait-objects.html) (trait
object). Обычно это выглядит как `Box<Error>`, либо `&Error`. Например, метод
`cause` возвращает `&Error`, который как раз является типаж-объектом. Позже мы
вернемся к применению `Error` как типаж-объекта.
...
ошибка работы с IО или ошибка парсинга строки в число. Определение ошибки может
...
impl error::Error for CliError {
    ...
    fn cause(&self) -> Option<&error::Error> {
        match *self {
            // N.B. В обоих случаях просходит неявное преобразование значения `err`
...
создает общий подход для конвертации *из* определенного типа `Т` в какой-то
другой тип (в данном случае, "другим типом" является имплементирующий тип, или
`Self`). Самое важное в типаже `From` — [множество его реализаций,
...
Итак, `From` полезен для выполнения преобразований между строками. Но как на
счет ошибок? Оказывается, существует одна важная реализация:

```rust,ignore
impl<'a, E: Error + 'a> From<E> for Box<Error + 'a>
```

Это реализация говорит, что *любой* тип, который реализует `Error`, можно
конвертировать в типаж-объект `Box<Error>`. Выглядит не очень впечатляюще, но
...
Настало время вернутся к нашему старому другу — макросу `try!`.
...
остается: `Box<Error>` не предоставляет никакой информации о сути ошибки. (N.B.
Это не совсем верно, поскольку в Rust есть инструменты рефлексии во время
выполнения, которые полезны при некоторых сценариях, но эти их рассмотрение
...
Чтобы исправить это, мы воспользуемся средством, с которым мы уже знакомы:
собственным типом ошибок. Давайте вспомним код, который считывает
...
конвертировать типы ошибок `io::Error` и `num::ParseIntError` к наш собственный
...
конструктора по значению. *Как правило*, это все что нужно.
...
от того, что вы предпримете, считается хорошей тоном обеспечить по крайней
...
представления ошибки (это вытекает из необходимости реализации `fmt::Debug` и
`fmt::Display`).

Кроме того, может быть полезным реализовать `From` для ваших типов ошибок. Это
позволяет вам (как автору библиотеки) и вашим пользователям
...
рекомендации. Это категорически *не* заповеди. Наверняка существуют веские
причины для того, чтобы нарушить любую из этих эвристик!
...
Те, для кого предназначен ваш пример должны осознавать, что необходимо
