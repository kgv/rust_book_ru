...
Лень является причиной этого! Следующее замыкание никогда не будет выполнено.
...
попробуем применить его для бесконечных итераторов, о которых мы упоминали
прежде:
...
iter](http://doc.rust-lang.org/std/iter/index.html).
