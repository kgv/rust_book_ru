...
*связывание имён*. Они связывают значение с именем, для того что бы
...
Вернёмся к связыванию. Связанные имен имеют область видимости — они
...
`fn main() {}`, в тоже время `y` доступен только во внутреннем блоке:
