...
Документация является важной частью любого программного проекта, и в Rust она
первоклассна. Давайте поговорим об инструментах Rust, предназначенных для
создания документации к проекту.
...
это. Примеры должны размещаться внутри блоков описания кода, о которых мы
сейчас поговорим, и могут иметь более одного раздела:
...
2. Будут вставлены некоторые общие `allow` атрибуты, в том числе:
   ...
   `dead_code`. Небольшие примеры часто вызывают эти строки.
...
Вообще, иногда, этого не хватает. Например, все эти примеры кода с `///`, о
...
библиотечные. Это связано с тем, как работает `rustdoc`: он компонует библиотеку в
