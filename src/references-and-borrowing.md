...
Вы также заметили что мы добавили звездочку (`*`) перед `y`. Это сделано из-за того, 
что `y` - это ссылка и сама данных не содержит. Вам нужно использовать (`*`), для того
чтобы получить доступ непосредственно к содержимому на которое указывает ссылка.
...
Ссылки не должны жить дольше, чем ресурс, на который они ссылаются. Rust
